...
export default function ContactSubmissionsTab() {
  ...
  const fetchSubmissions = async () => {
    ...
    try {
      ...
      if (typeFilter && typeFilter !== 'All Inquiry Types') query.append('type', typeFilter);

      const res = await fetch(`http://localhost:5000/api/contact-submissions?${query.toString()}`);
      if (res.ok) {
  ...
  const handleStatusChange = async (id: number, newStatus: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/contact-submissions/${id}/status`, {
        method: 'PUT',
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/contact-submissions/${id}`, {
        method: 'DELETE'
      });
      if (res.ok) {
    ...
    <>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-[600px] overflow-hidden flex flex-col">
            ...
            <div className="px-6 pt-6 pb-4 relative">
              ...
              <div className="inline-block bg-[#ffeee6] text-[#e35a1a] px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider mb-3">
                {viewLead.inquiry_type || 'Editorial'}
              </div>
              <h2 className="text-2xl font-bold text-[#0B1E36]">Query Detail Submission</h2>
            ...
            <div className="px-6 py-4 flex-1 overflow-y-auto">
              <div className="grid grid-cols-2 gap-y-6 gap-x-4 mb-8">
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Submitter Name</label>
                ...
                <div>
                  ...
                  <div className="font-semibold text-gray-900">{viewLead.company || 'N/A'}</div>
                </div>
                <div>
                ...
                <div>
                  ...
                  >
                    {['New', 'In progress', 'Resolved', 'Archived'].map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                </div>
                <div>
              ...
              <div>
                <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Question / Message Details</label>
                <div className="bg-gray-50 border border-gray-100 rounded-lg p-4 text-sm text-gray-700 whitespace-pre-wrap min-h-[100px]">
                  {viewLead.message || 'No details provided.'}
                </div>
              </div>
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h2 className="text-[22px] font-bold text-[#0B1E36]">Contact Us Submissions</h2>
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <div className="relative">
              ...
              <input 
                ...
                className="pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm w-full sm:w-[280px] focus:outline-none focus:ring-2 focus:ring-[#215EBA]/20 focus:border-[#215EBA]"
              />
            </div>
            <select 
              className="px-4 py-2 border border-gray-200 rounded-lg text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#215EBA]/20 focus:border-[#215EBA] bg-white appearance-none"
              value={typeFilter}
          ...
          <table className="w-full text-left text-sm text-gray-600 min-w-[800px]">
            <thead className="bg-gray-50 text-[11px] font-bold uppercase tracking-wider text-gray-500">
              <tr>
                ...
                <th className="px-6 py-4 w-[250px]">MESSAGE</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr>
                  <td colSpan={8} className="px-6 py-8 text-center text-gray-500">Loading submissions...</td>
                </tr>
                ...
                submissions.map((sub) => (
                  <tr key={sub.id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-6 py-5 whitespace-nowrap">
                      ...
                      <div className="text-gray-500 text-xs mt-0.5">{sub.company || 'N/A'}</div>
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap text-gray-600">{sub.email}</td>
                    <td className="px-6 py-5 whitespace-nowrap">
                      <div className="text-gray-600 text-xs"><span className="text-gray-400 mr-1">P:</span> {sub.phone || '000 000 0000'}</div>
                      <div className="text-gray-600 text-xs mt-0.5"><span className="text-gray-400 mr-1">W:</span> {sub.whatsapp || '000 000 0000'}</div>
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap">
                      <span className="bg-gray-100 text-[#0B1E36] px-3 py-1 rounded-full text-xs font-medium">
                        {sub.inquiry_type || 'Editorial'}
                      </span>
                    </td>
                    <td className="px-6 py-5">
                      <p className="text-gray-600 line-clamp-1">{truncateMessage(sub.message)}</p>
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap relative">
                      <div 
                        className="cursor-pointer border border-[#215EBA]/20 text-[#215EBA] px-4 py-1.5 rounded text-xs font-medium flex items-center justify-between gap-3 min-w-[120px]"
                      ...
                      {openDropdownId === sub.id && (
                        <div className="absolute top-12 left-6 w-[120px] bg-white border border-[#215EBA] rounded shadow-lg z-10 overflow-hidden">
                          {['New', 'In progress', 'Resolved', 'Archived'].map((statusOption) => (
                            <div 
                              key={statusOption}
